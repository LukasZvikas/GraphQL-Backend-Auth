...
const { INVALID_EMAIL_ERROR } = require('../errorTypes');
...
describe('Reset pass', () => {
    ...
    let incorrectToken = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjVjZGM4MjBjZjVkMzUzOWYyNWM4YzRmZCIsImVtYWlsIjoidGVzdEBnbWFpbC5jb20iLCJpYXQiOjE1NTgxMjM0Nzd9.CmXk438Aw9_fvV8KIUoxyfdP597xneRfZMSy47J83';

    let incorrectEmail = 'test1@gmail.com';
    ...
    it('if a user exists, sends an password reset link with token', (done) => {
        AuthQueries.sendResetPassLink(null, { email: userDetails.email}).then((passResetObj) => {
          assert(passResetObj.token);
          token = passResetObj.token;
          done();
        });
    });
    ...
    it('resets password if correct token is provided in a link', (done) => {
      AuthMutations.resetPass(null, {token, newPassword: 'password1'}).then((passResetObj) => {
    ...
    it('throws an error if incorrect token is provided in a link', (done) => {
      AuthMutations.resetPass(null, {token: incorrectToken, newPassword: 'password1'}).then((passResetObj) => {
        done();
      }).catch((err) => {
